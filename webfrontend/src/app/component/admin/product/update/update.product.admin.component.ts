...
import { MauBangImage } from '../../../../model/maubangimage';
...
export class UpdateProductAdminComponent implements OnInit {
  ...
  constructor(
    ...
    this.productId = 0;
    this.product = {} as MauBang;
    this.updatedProduct = {} as MauBang;  
  }
  ...
  getProductDetails(): void {
    this.productService.getDetailProduct(this.productId).subscribe({
      next: (apiResponse: ApiResponse) => {
        ...
        this.updatedProduct.product_images.forEach((product_image:MauBangImage) => {
          product_image.image_url = `${environment.apiBaseUrl}/maubangs/images/${product_image.image_url}`;
        });
      },
      complete: () => {
  ...
  deleteImage(productImage: MauBangImage) {
    if (confirm('Are you sure you want to remove this image?')) {
      ...
      this.productService.deleteProductImage(productImage.id).subscribe({
        next:(productImage: MauBangImage) => {
          location.reload();          
        },        
        error: (error: HttpErrorResponse) => {
          debugger;
